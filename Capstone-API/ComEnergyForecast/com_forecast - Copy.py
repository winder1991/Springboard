...
def adjust_time(current_datetime, num_year):
    with open(model_path + 'timestamp.pickle', 'rb') as f:
        df_sample_timestamp = pickle.load(f)
    time_loc = len(df_sample_timestamp)
    if df_sample_timestamp[len(df_sample_timestamp)-1] == current_datetime.replace(year=2019):
        current_year = current_datetime.year
        df_sample_timestamp[len(df_sample_timestamp)-1].replace(year=current_year)
        current_all_year =[each.replace(year=(current_year-1)) for each in df_sample_timestamp if each != current_datetime]
        current_all_year.append(current_datetime)
    else:
        end_date = current_datetime.replace(year=2018)
        current_year = current_datetime.year
        time_loc = list(df_sample_timestamp).index(end_date)
        second_half_time = list(df_sample_timestamp)[0:(time_loc+1)]
        first_half_time = list(df_sample_timestamp)[(time_loc+1):]
...
#             df_input['weekday'] = df_input['ds'].apply(is_weekday)
#             df_input['weekend'] = ~df_input['ds'].apply(is_weekday)
...
#             df_input = pd.DataFrame({'ds':timestamp, 'y':center_value, 'Temperature':temp_hist})
            ...
            m.add_seasonality(name='weekly_on_summer', period=7, fourier_order=tuned_param[0]['weekly_seasonality'].iloc[0], condition_name='summer')
            m.add_seasonality(name='weekly_on_winter', period=7, fourier_order=tuned_param[0]['weekly_seasonality'].iloc[0], condition_name='winter')
#             m.add_seasonality(name='weekly_weekday', period=7, fourier_order=tuned_param[0]['weekly_seasonality'].iloc[0], condition_name='weekday')
#             m.add_seasonality(name='weekly_weekend', period=7, fourier_order=tuned_param[0]['weekly_seasonality'].iloc[0], condition_name='weekend')
            m.fit(df_input)
...
#             future['Temperature']= temperature[temp_index]
        ...
        for i in range(len(building_ids)):
            # if len(df_cluster_items['std'].iloc[i])>1 and days_in_future<30:
            #     month = timestamp[-1].month
            #     dev_i = df_cluster_items['std'].iloc[i][month-1]
            # else:
            #     dev_i = df_cluster_items['std'].iloc[i][0]
